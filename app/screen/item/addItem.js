...
import { add_item } from '../../redux/actions/itemAction';
...
class AddItem extends React.PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            input_item_name: null,
            input_item_group: null,
    ...
    render() {
        ...
        let { input_min_price, input_max_price, item_price_array, add_button_loading } = this.state;

        const images = [];
        const { item_image } = this.state;
        item_image.map(img => images.push({ url: img.uri }));
                ...
                <ScrollView>
                    ...
                    <View style={{ marginHorizontal: 15 }}>
                        <TextInput style={{ color: '#222', paddingHorizontal: 10, minHeight: 45, borderWidth: 1, borderColor: '#9E9E9E', borderRadius: 5, marginBottom: 10 }}
                            placeholder={"Item Name"} onChangeText={(input) => { this.setState({ input_item_name: input }) }} />
                        {
                            Platform.OS === 'android' ?
                        ...
                        <TextInput style={{ color: '#222', paddingHorizontal: 10, borderWidth: 1, borderBottomWidth: 1, borderColor: '#9E9E9E', borderRadius: 5, marginBottom: 10, minHeight: 70 }}
                            ...
                            multiline={true} placeholder={"Description(Optional)"} onChangeText={(input) => { this.setState({ input_description: input }) }} />
                        ...
                        <View style={{ minHeight: 100, borderWidth: 1, borderRadius: 5, borderColor: '#9E9E9E', padding: 10 }}>
                            <View style={{ flexDirection: 'row', marginBottom: 10, justifyContent: 'center', alignItems: 'center' }} >
                                ...
                                <Text style={{ flex: 0.7, color: '#f09839', fontSize: 16, fontWeight: 'bold' }}>Unit</Text>
...

                            {
                                item_price_array.length > 0 ?
                                    // <FlatList
                                    //     data={item_price_array}
                                    //     renderItem={this.render_item_price}
                        ...
                        </View>
                        {
                            add_button_loading ?
                                ...
                                <TouchableOpacity style={styles.loginBtn} onPress={() => this.confirm()}>
                                    <View><Text style={{ color: '#FFF', fontSize: 16 }}>Add</Text></View>
                                </TouchableOpacity>
                        }
    ...
    add_price = () => {
        ...
        let { input_min_price, input_max_price, input_currency, input_unit, } = this.state;
        ...
        if (input_min_price && input_max_price && input_currency && input_unit) {
            ...
            console.log(input_min_price, input_max_price, input_currency, input_unit);
    ...
    //     if (input_min_price && input_max_price && input_currency && input_unit) {
    //         var unit_found = this.state.item_price_array.find(({ unit, currency }) => unit == input_unit && currency == input_currency)
    //         if (!unit_found) {
    //             if (Number(input_min_price) < (input_max_price)) {
    //                 let price = {
    //                     "min_price": input_min_price,
    ...
    //             }
    //         } else {
    //             alert(`Already filled price for ${input_currency} with ${input_unit}`)
    //         }
    ...
    delete_item(item) {
        ...
        const { item_price_array } = this.state;
        const index = item_price_array.indexOf(item);
        const item_price_list = item_price_array;
        if (index > -1) {
            item_price_list.splice(index, 1);
        }
        if (item_price_list.length == 0 || null) {
            this.setState({ item_price_added: false })
        }
        this.setState({ item_price_array: item_price_list });
        this.forceUpdate();
        ...
        return (
            <View key={index} style={{ flexDirection: 'row', marginBottom: 10 }} >
                ...
                <View style={{ flex: 0.7, flexDirection: 'row' }} >
                    <Text style={{ fontSize: 14, fontWeight: 'bold', flex: 0.7 }}>{item.unit}</Text>
                    {/* <Text style={{ fontSize: 14, fontWeight: 'bold',flex:1,textAlign:"right" }}>Unit </Text> */}
                    <TouchableOpacity onPress={() => this.delete_item(item)}><DeleteIcon name="delete" size={20} color="red" style={{ flex: 1, textAlign: "right", }} /></TouchableOpacity>
                    {/* <TouchableOpacity onPress={() => this.delete_confirm(item)}><DeleteIcon name="delete" size={20} color="red" style={{ flex: 1, textAlign: "right" }} /></TouchableOpacity> */}
                </View>
            </View>
        )
    }
    ...
    confirm() {
        ...
        const { image_selected, input_item_name, input_item_group, input_description, item_price_array, item_image, item_price_added } = this.state;

        if (input_item_name && item_price_added && image_selected && input_item_group) {
            ...
            this.props.add_item(data, () => this.props.navigation.goBack());
...
const mapDispatchToProps = dispatch => ({
    add_item: (data, callback) => {
        dispatch(add_item(data, callback));
    },
})
