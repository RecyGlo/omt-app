    ...
    render_item = ({ item }) => {
        ...
        return (
            <TouchableOpacity style={{ backgroundColor: '#FFFFFF', elevation: 1, borderBottomWidth: 1, borderBottomColor: '#E2E2E2', flexDirection: 'row', alignItems: 'center', padding: 10, }}
                onPress={() => item.category.includes('MarketPlace_') ? this.click_notification(item) : this.props.navigation.navigate('NotificationDetail', { notification: item })}>
                <View style={{ flexDirection: 'column', marginLeft: 10, flex: 1, overflow: 'hidden', padding: 10 }}>
                    <Text style={{ fontSize: 16 }}>{item.category.includes('MarketPlace_') ? item.message : item.title}</Text>
