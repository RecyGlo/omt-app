...
class MarketPlaceUploadedList extends React.PureComponent {
    ...
    componentDidMount() {
        this.props.get_uploaded_market_product_list(this.props.user._id);
        ...
        return (
            <TouchableOpacity style={{ backgroundColor: '#FFFFFF', elevation: 1, borderRadius: 5, marginVertical: 5, marginHorizontal: 10, flexDirection: 'row', alignItems: 'center', padding: 10, flex: 1 }}
                ...
                <View style={{ flexDirection: 'column', marginLeft: 10, flex: 5, overflow: 'hidden' }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}><Text style={{ backgroundColor: '#FFF', color: '#222', padding: 5, marginLeft: 2, fontSize: 13, fontStyle: 'italic' }}>{item.price} MMK</Text>
                        {
...

                    {
                        show_loading ?
                            ...
                            uploaded_market_product_list.length > 0 ?
                                ...
                                :
                                <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                                    <Text style={{ fontSize: 17, fontWeight: 'bold' }}>No Uploaded Product By You</Text>
                                </View>
                    }
...
export default connect(mapStateToProps, mapDispatchToProps)(MarketPlaceUploadedList);